...
if choice == "Modelling": 
    ...
    if st.button('Run Modelling'): 
        ...
        if len(df_preprocessed) > 1:
            setup(df_preprocessed, target=chosen_target, train_size=min(0.7, len(df_preprocessed) - 1))
            setup_df = pull()
...
if choice == "Elasticized Auto-Modelling":
    ...
    if file: 
        ...
        st.warning("No test data changes. Retraining the model with train  data...")
        # Auto-Trigger model training (One-Shot EAML Cycle)
        df_preprocessed = preprocess_data(df)
        if len(df_preprocessed) > 1:
            setup(df_preprocessed, target=chosen_target, train_size=min(0.7, len(df_preprocessed) - 1))
            setup_df = pull()
            ...
            st.error("Not enough samples in the dataset to auto-perform modelling.")
